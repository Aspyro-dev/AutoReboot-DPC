...
fun AutoRebootScreen() {
    ...
    fun formatConfiguredDuration(ms: Long): String {
        ...
        return when {
            ...
            minutes % 60L == 0L -> {
                ...
                "$hours hour" + if (hours == 1L) "" else "s" +
                    " $remainingMinutes minute" + if (remainingMinutes == 1L) "" else "s"
            }
        }
    }
    ...
    Column(
        ...
        Box(Modifier.padding(top = 4.dp)) {
            Button(onClick = { timerMenuExpanded = true }) {
                Text("Timer: ${formatConfiguredDuration(configuredDuration)}")
            }
            DropdownMenu(
                ...
                options.forEach { duration ->
                    DropdownMenuItem(
                        text = { Text(formatConfiguredDuration(duration)) },
                        onClick = {
        ...
        if (AutoRebootState.lastScheduleError(context).isNotEmpty()) {
            Text(
                "Schedule error: ${AutoRebootState.lastScheduleError(context)}",
                Modifier.padding(top = 8.dp)
            )
        }
        Button(
