...
fun AutoRebootScreen() {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
        ...
        Button(onClick = {
            ...
            if (!enabled) { AutoRebootState.clearTimer(context); RebootScheduler.cancel(context) }
            refresh = !refresh
        }, Modifier.padding(top = 16.dp)) { Text(if (AutoRebootState.isEnabled(context)) "Disable" else "Enable") }
        ...
        Button(onClick = {
            ...
            refresh = !refresh
        }, Modifier.padding(top = 8.dp)) { Text("Start 1-minute test timer") }
    }
}
